...
class DataFeed:
    ...
    def get_today_expiry_str(self) -> str:
        """Today's expiry as DD-MM-YYYY; rolls to tomorrow past SESSION_END."""
        now    = _utcnow()
        rolled = False
...
class PositionManager:
    ...
    def open_straddle(self, call_product: dict,
        ...
        log.info("=" * 55)
        log.info(f"[PositionMgr] 📌 PAPER SELL Straddle — Trade #{trade_id}")
...
class OrderManager:
    ...
    # ── LIVE METHODS ───────────────────────────────────────────────────────
    # Uncomment and swap in place of paper_ calls to go live.
    ...
    def monitor_loop(self, trade: Trade):
        ...
        while trade.is_open() and self.state.running:
            ...
            if self.strategy.past_session_end():
                ...
                for leg in [trade.call_leg, trade.put_leg]:
                    self.orders.paper_buy(leg.product_id, leg.symbol,
                                          QUANTITY, leg.ltp)
    ...
    def run(self):
        ...
        while self.state.running:
            ...
            if enter:
                ...
                self.orders.paper_sell(call_prod["product_id"], call_sym,
                                       QUANTITY, trade.call_leg.entry_price)
                self.orders.paper_sell(put_prod["product_id"],  put_sym,
                                       QUANTITY, trade.put_leg.entry_price)
